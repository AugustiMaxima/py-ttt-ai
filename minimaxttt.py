#fully empty deck takes around 9 seconds to finish
#for covenience and performance reasons, empty deck strictly require empty string ""
#transplanted the minimax ai to my older tic-tac-toe code from lab8. since the original
#version had amazing modularity and is well-structured and implemented
def TreeBuild(S,player):
        op="";mm=0;index=-1;query=[];l=999
        vic=win(S)
        if vic=="X":
        ...
        for i in range(9):
                if S[i]=="":
                        v=TreeBuild(S[:i]+[player]+S[i+1:],op)
                        query+=[v]
                        if player=="X" and mm<v[2]:
                                ...
                                mm=v[2];index=i;
        if index==-1:
                return [S,player,0,(index%3,index//3),[]]
